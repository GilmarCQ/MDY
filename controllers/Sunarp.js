...
const getPJRazonSocial = (req, res) => {
    ...
    axios.get(`${URL_API_SUNARP}/PJRazonSocial?razonSocial=${body.nombre}`)
        .then(response => {
            res.status(200).json({
                ok: true,
...
const getOficinas = (req, res) => {
    const body = req.query;
    axios.get(`${URL_API_SUNARP}/Oficinas`)
        .then(response => {
            res.status(200).json({
                ok: true,
                contenido: response.data
...
const getAsientos = (req, res) => {
    const body = req.query;
    axios.get(`${URL_API_SUNARP}/ListarAsientos?zona=${body.zona}&oficina=${body.oficina}&partida=${body.partida}&registro=${body.registro}`)
        .then(response => {
            res.status(200).json({
                ok: true,
                contenido: response.data
...
const getAsiento = (req, res) => {
    const body = req.query;
    axios.get(`${URL_API_SUNARP}/VerAsientos?transaccion=${body.transaccion}&idImg=${body.idImg}&tipo=${body.tipo}&nroTotalPag=${body.totalPag}&nroPagRef=${body.pagRef}&pagina=${body.pagina}`)
        .then(response => {
            res.status(200).json({
                ok: true,
                contenido: response.data
...
const getVehiculo = (req, res) => {
    const body = req.query;
    axios.get(`${URL_API_SUNARP}/VerDetalleRPV?zona=${body.zona}&oficina=${body.oficina}&placa=${body.placa}`)
        .then(response => {
            res.status(200).json({
                ok: true,
                contenido: response.data
